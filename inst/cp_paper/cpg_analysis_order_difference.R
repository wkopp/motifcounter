...
variants=data.frame( markov=c(rep(0,1),rep(1,1),rep(2,1)),
                    name=rep(c("sp1"),3),
                    type=rep(c("transfac"),3))

scorelabnames1=c(expression("P"["E,0"]^"*"*"(S)"),
                 expression("P"["E,1"]^"*"*"(S)"), expression("P"["E,2"]^"*"*"(S)"))
scorelabnames2=c(expression("P"["0"]*"(S)"),
                 expression("P"["0"]*"(S)"), expression("P"["0"]*"(S)"))
countlabnames1=c(expression("P"["E,0"]^"*"*"(X)"),
                 expression("P"["E,1"]^"*"*"(X)"),expression("P"["E,2"]^"*"*"(X)"))
countlabnames2=c(expression("P"["CP,0"]^"N"*"(X)"),
                 expression("P"["CP,0"]^"N"*"(X)"), expression("P"["CP,0"]^"N"*"(X)"))
...
for (var in 1:nrow(variants)) {
  ...
  read.background.sampling(seqfile,variants$markov[var])
	...
	#simres= sim.scores(ncol(pwm),10000000)
	simres= sim.scores(10000000,1)
	...
	df=data.frame(score=c(simres[[1]],result[[1]]),
        ...
        postscript(file=sprintf("%s/scoredist_%s_d%d.eps",figdir, pwmname,
                                                variants$markov[var]),
						height=4,width=6)
...
#																expression("P"["Analytic"]*"(S)"*sprintf(" (order-%d)",variants$markov[var]))))+
	...
	postscript(file=sprintf("%s/scoredist_%s_d%d_part.eps",figdir, pwmname,
					variants$markov[var]),
					height=4,width=6)
	...
	reldiff[var]=sum(simres[[2]][r])/sum(result[[2]][r])
	...
	postscript(file=sprintf("%s/cntdist_%s_d%d.eps",figdir,pwmname,variants$markov[var]),
						height=4,width=6)
