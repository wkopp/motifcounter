...
#							cppape=cpdist.pape[[1]][1:maxhits],
							bayes=bayes[[1]][1:maxhits],
...
pl=ggplot(df,aes(x=hits,y=prob, col=Model))+
  ...
  #geom_line()+
  pl=pl+geom_point()
pl<-pl+geom_point(data=df, aes(x=hits,y=bayes,col="Bayes"), size=3)
...
signif(sum(abs(df2$prob-df2$bayes)),digits=r),
signif(sum(abs(df2$prob-df2$cpnew)), digits=r),
signif(sum(abs(df2$prob-df2$bin)),digits=r),
signif(sum(abs(df2$prob[qsig]-df2$bayes[qsig])),digits=r),
