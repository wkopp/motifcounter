...
int initExtremalScore(ExtremalScore *s, double dx, int length, int order) {
  ...
  #else
  s->maxforward=calloc((length)*power(ALPHABETSIZE, order), sizeof(int));
  s->maxbackward=calloc((length)*power(ALPHABETSIZE, order), sizeof(int));
  s->minforward=calloc((length)*power(ALPHABETSIZE, order), sizeof(int));
  s->minbackward=calloc((length)*power(ALPHABETSIZE, order), sizeof(int));
  s->intervalstart=calloc((length)*power(ALPHABETSIZE, order), sizeof(int));
  s->intervalend=calloc((length)*power(ALPHABETSIZE, order), sizeof(int));
  #endif
...
// computes the minimum or maximum score for a pwm, the stationary distribution and a precomputed
// escore.
...
// for the remaining M-k nucleotids, with a certain nucleotid observed at position k-1.
...
void extremScoresPerPositionBack(int max, DMatrix *theta, double *trans, 
  ...
  if (order>theta->nrow) {
    ...
    #else
    fprintf(stderr,"Order is cannot be higher than the motif length!\n");
    #endif
    return;
  }
  ...
  for (i=theta->nrow-1;i>=order && i>0; i--) {
    for (m1=0; m1<power(ALPHABETSIZE, order); m1++) {
      ...
      extrema[power(ALPHABETSIZE, order)*(i-1)+m1]=getExtrem(s,ALPHABETSIZE, max);
    }
  }
}

void printValues(int *v, int len) {
  #ifndef IN_R
  int i;
  for (i=0; i<len; i++) {
    printf("%d ", v[i]);
...
// that is, what is the maximum/minimum score after k nucleotids having an certain
...
void extremScoresPerPositionForward(int max, DMatrix *theta, 
  ...
  // the results of the function are used to compute the actual memory requirements
  // for the current motif, background, and chosen threshold
   if (order>theta->nrow) {
     #ifdef IN_R
  ...
  #else
  if (order>1) {
    s=calloc(power(ALPHABETSIZE,order), sizeof(int));
  } else {
    s=calloc(ALPHABETSIZE, sizeof(int));
  }
  #endif
  ...
  for (;i<theta->nrow; i++) {
    for (m1=0; m1<power(ALPHABETSIZE, order); m1++) {
      //getAssignmentFromIndex(m1, order+1, ass);
  ...
  //    printValues(s,4);
      ...
      for (k=0; k<ALPHABETSIZE;k++) {
        if (order==0) {
          ...
          s[k]+=extrema[(i-1)*power(ALPHABETSIZE, order)+m2+power(ALPHABETSIZE,order-1)*k];
        }
      }
     // printValues(s,4);
     // fprintf(stdout,"\n");
      extrema[i*power(ALPHABETSIZE, order)+m1]=getExtrem(s, ALPHABETSIZE, max);
    }
  }
  #ifdef IN_R
  Free(s);
...
void loadIntervalSize(ExtremalScore *escore, int *threshold) {
  ...
  for (; j<escore->len; j++) {
    for (i=0; i<power(ALPHABETSIZE, order); i++) {
      ...
      if (threshold!=NULL && lower< (*threshold-escore->maxbackward[j*power(ALPHABETSIZE, order) +i])) {
        lower=*threshold-escore->maxbackward[j*power(ALPHABETSIZE, order) +i];
      } else if (threshold!=NULL && escore->maxbackward[j*power(ALPHABETSIZE, order) +i]==
                                    escore->minbackward[j*power(ALPHABETSIZE, order) +i]) {
        lower=*threshold;
      }
      upper=escore->maxforward[j*power(ALPHABETSIZE, order) +i];
      if (threshold!=NULL && upper>(*threshold -escore->minbackward[j*power(ALPHABETSIZE, order) +i])) {
        upper=*threshold-escore->minbackward[j*power(ALPHABETSIZE, order) +i];
      } else if (threshold!=NULL && escore->maxbackward[j*power(ALPHABETSIZE, order) +i]==
                                    escore->minbackward[j*power(ALPHABETSIZE, order) +i]) {
        upper=*threshold;
      }
      //if (upper==lower) upper=lower=*threshold;
...
int getScoreLowerBound(ExtremalScore *escore, int pos, int nucindex) {
  return escore->intervalstart[pos*power(ALPHABETSIZE, escore->order) + nucindex];
}

int getScoreUpperBound(ExtremalScore *escore, int pos, int nucindex) {
  return escore->intervalend[pos*power(ALPHABETSIZE, escore->order) + nucindex];
}

int getScoreLowerBoundUnconstrainted(ExtremalScore *escore, int pos, int nucindex) {
  return escore->minforward[pos*power(ALPHABETSIZE, escore->order) + nucindex];
}

int getScoreUpperBoundUnconstrainted(ExtremalScore *escore, int pos, int nucindex) {
  return escore->maxforward[pos*power(ALPHABETSIZE, escore->order) + nucindex];
}

int getTotalScoreLowerBound(ExtremalScore *escore) {
  return getMin(&escore->intervalstart[(escore->len-1)*power(ALPHABETSIZE, escore->order)],
                                power(ALPHABETSIZE, escore->order));
}
int getTotalScoreUpperBound(ExtremalScore *escore) {
  return getMax(&escore->intervalend[(escore->len-1)*power(ALPHABETSIZE, escore->order)],
                                power(ALPHABETSIZE, escore->order));
}
int * getLastScoreLowerBound(ExtremalScore *escore) {
  return &escore->intervalstart[(escore->len-1)*power(ALPHABETSIZE, escore->order)];
}
...
int getScoreLowerBoundPos(ExtremalScore *escore, int pos) {
  return getMin(&escore->intervalstart[pos*power(ALPHABETSIZE, escore->order)],
   power(ALPHABETSIZE, escore->order));
}
...
void loadMinMaxScores(DMatrix *pwm, double *station, double *trans, ExtremalScore *e) {
   ...
   if (e->order>pwm->nrow) {
    ...
    minScoresPerPositionForward(pwm,station, trans,e->minforward, &e->dx, e->order);
    minScoresPerPositionBack(pwm,trans,e->minbackward, &e->dx, e->order);
    maxScoresPerPositionForward(pwm,station, trans,e->maxforward, &e->dx, e->order);
...
#ifndef IN_R
void testmax(char *pwmfile, char *bgfile, char *output, char *gran) {
  int i, Nmotif;
  DMatrix *pwm, *cpwm;
...
//      fread(&seql[i],sizeof(int),1,f);
    ...
    f =fopen(pwmfile,"rb");
    if (f==0) {
      fprintf(stderr, "pwm bin file could not be opened!\n");
      return;
    }
    for (i=0;i<5000;i++) {
    ...
    // 02231023
     ...
     //printAllScores(pwm, stationary, trans,&dx,  order);
     //printAllScores(cpwm, stationary, trans,&dx,  order);
    ...
    memset(&mscore,0, sizeof(MotifScore1d));
