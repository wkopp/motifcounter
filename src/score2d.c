...
int initScoreDistribution2d (DMatrix *theta, double *bg1, MotifScore2d *result, int order) {
  int i;
...
void ShiftMultiplyScoreIndex2d(Score2d *dest, Score2d *src, 
...
#ifndef IN_R
#ifdef DEBUG
  fprintf(stdout, "dsf=%d, fp=[%d/%d] fc=[%d/%d] ",*fds,flowerprev,fupperprev,flowercur,fuppercur);
 ...
 // src->start, src->end, lprev,uprev,lcur,ucur);
  ...
  if (dest->start1<0) {
   // dest->end1-=dest->start1;
    leftshift1=-dest->start1;
    dest->start1=0;
  }
  if (dest->start2<0) {
   // dest->end2-=dest->start2;
  ...
  if (dest->start1 > dest->end1 && frestinterval<=0) { return; }
  if (dest->start2 > dest->end2 && rrestinterval<=0) { return; }

#ifndef IN_R
#ifdef DEBUG
  fprintf(stdout, "src1=[%d,%d], dest1=[%d/%d] lshift=%d, rest=%d ",src->start1,src->end1,dest->start1,dest->end1, leftshift1,frestinterval);
  fprintf(stdout, "src2=[%d,%d], dest2=[%d/%d] lshift=%d, rest=%d\n",src->start2,src->end2,dest->start2,dest->end2, leftshift2,rrestinterval);
  #endif
  #endif
  #ifdef PARALLEL_WK
  ...
  for (r=dest->start2; r<=dest->end2;r++) {
    for (f=dest->start1; f<=dest->end1;f++) {
      dest->y[r*srange + f]=src->y[(r-dest->start2+src->start2+leftshift2)*srange +
  ...
  for (r=dest->start2; r<=dest->end2;r++) {
    for (f=0; f<frestinterval; f++) {
      dest->y[r*srange+dest->end1]+=src->y[(r-dest->start2 + src->start2+leftshift2)*srange +
  ...
  for (f=0; f<frestinterval; f++) {
    for (r=0; r<rrestinterval; r++) {
      dest->y[dest->end2*srange + dest->end1]+=
        src->y[(r +src->start2 + dest->end2 -dest->start2+1+leftshift2)*srange +
          src->start1 + dest->end1 - dest->start1 +1 +f+leftshift1]*p;
  //    testp+=src->y[(src->start2 + dest->end2 -dest->start2+1+r)*srange +src->start1 + dest->end1 - dest->start1 +1 +f]*p;
    }
  }
}

#define MAXBFLEN 12
int computeSeqenByBruteForce(int mlen1, int mlen2, int shift, ExtremalScore *escore1, ExtremalScore *escore2) {
  int steps=0, psteps;
  int i, prefix, j;
  int max, corder;
  int imin, start;
  ...
  for (j=corder-1; j<mlen2; j++) {
    if (shift+j>=mlen1) {
      for (prefix=0; prefix<power(ALPHABETSIZE, escore1->order); prefix++) {
        psteps+=(getScoreUpperBound(escore2,j,prefix)-getScoreLowerBound(escore2,j,prefix)+1)*ALPHABETSIZE;
      }
    } else {
      for (prefix=0; prefix<power(ALPHABETSIZE, escore1->order); prefix++) {
        psteps+=(getScoreUpperBound(escore2,j,prefix)-getScoreLowerBound(escore2,j,prefix)+1)*
         (getScoreUpperBound(escore1,shift+j,prefix)-getScoreLowerBound(escore1,shift+j,prefix)+1)*ALPHABETSIZE;
      }
    }
  }
  //fprintf(stdout, "i=%d, steps=%d\n",corder-1,psteps);
  imin=corder-1;
  ...
  for (i=start; i<max; i++) {
    ...
    for (j=i+1; j<max; j++) {
      ...
      // final 1 dimensional convolution
        for (prefix=0; prefix<power(ALPHABETSIZE, escore1->order); prefix++) {
          steps+=(getScoreUpperBound(escore2,j-shift,prefix)-getScoreLowerBound(escore2,j-shift,prefix)+1)*ALPHABETSIZE;
        }
      } else {
        for (prefix=0; prefix<power(ALPHABETSIZE, escore1->order); prefix++) {
          steps+=(getScoreUpperBound(escore2,j-shift,prefix)-getScoreLowerBound(escore2,j-shift,prefix)+1)*
          (getScoreUpperBound(escore1,j,prefix)-getScoreLowerBound(escore1,j,prefix)+1)*ALPHABETSIZE;
        }
      }
      if (steps>psteps) { break; }
    }
    if (steps<psteps) {
      imin=i;
      psteps=steps;
...
int computeTotalScoreDistribution2d(MotifScore2d *mscore, int order) {
  int i;

  mscore->cprob=0.0;

  for (i=0; i<power(ALPHABETSIZE, order); i++) {
    mscore->cprob+= mscore->ScoreBuffer1[i].y[0];
  }
  return 0;
}
void printProb(Score2d *s, int len) {
  #ifndef IN_R
  double p=0.0;
  int i=0, j=0;
  for (i=s->start1;i<=s->end1;i++) {
    for (j=s->start2;j<=s->end2; j++) {
      p+=s->y[j*len+i];
      if (s->y[j*len+i]>0.0) {
...
int computeScoreDistribution2DDP_init(DMatrix *pwm1, DMatrix *pwm2,
  ...
  if (nsubmotif==shift+order) {
      ...
      for (; k<K; k++) {
        if (shift+order>pwm1->nrow) {
          ...
          for (x=0; x< power(ALPHABETSIZE, shift+order-pwm1->nrow); x++) {
            jointindex=x*power(ALPHABETSIZE,pwm1->nrow-shift)+k/power(ALPHABETSIZE, shift+order-pwm1->nrow);

            pinit=&init1d->ScoreBuffer1[(pwm1->nrow-1)*power(ALPHABETSIZE, order)+jointindex];
            if (pinit->start<pinit->end) continue;
            pext= pinit->y[0];
            for (l=0; l<shift+order-pwm1->nrow; l++) {
              jointindex*=ALPHABETSIZE;
              inter=k-(k/power(ALPHABETSIZE, shift+order-pwm1->nrow-l))*power(ALPHABETSIZE, shift+order-pwm1->nrow-l);
        ...
        #ifndef IN_R
        #ifdef DEBUG
        fprintf(stdout, "post-init2DDP-extention mode: nsub=%d m=0 i=%d ", nsubmotif, i);
        printProb(&mscore->ScoreBuffer1[i], mscore->meta.length);
          ...
          // score threshold
          mscore->ScoreBuffer1[i].start1 =init1d->ScoreBuffer1[pos*power(ALPHABETSIZE, order)+i].start;
          mscore->ScoreBuffer1[i].end1 =init1d->ScoreBuffer1[pos*power(ALPHABETSIZE, order)+i].end;
          mscore->ScoreBuffer1[i].start2 =0;
          mscore->ScoreBuffer1[i].end2 =0;
          ...
          for (s1=init1d->ScoreBuffer1[pos*power(ALPHABETSIZE, order)+i].start;
            ...
            mscore->ScoreBuffer1[i].y[s1]= init1d->ScoreBuffer1[pos*power(ALPHABETSIZE, order)+i].y[s1];
          }
        #ifndef IN_R
        #ifdef DEBUG
        fprintf(stdout, "init2DDP-normal mode: nsub=%d m=0 i=%d ", nsubmotif, i);
        printProb(&mscore->ScoreBuffer1[i], mscore->meta.length);
        #endif
        #endif
        }
    ...
    // in this case 2DBF is used. It has to be computed prior to using it here.
    for (i=0; i<power(ALPHABETSIZE, order); i++) {
...
int computeScoreDistribution2DDP(DMatrix *pwm1, DMatrix *pwm2,
  ...
  double pf;
  int corder=order;
  int score1[power(ALPHABETSIZE, order+1)], score2[power(ALPHABETSIZE, order+1)];
  ...
  for (m=nsubmotif; m<pwm1->nrow+shift;m++) {
    for (i=0; i<power(ALPHABETSIZE, order); i++) {
      if (m<pwm1->nrow) {
        getScoresIndex(&pwm1->data[(m)*ALPHABETSIZE],&trans[i*ALPHABETSIZE], 
      ...
      //fprintf(stdout,"second s1=%d, s2=%d\n",score1[0],score2[0]);

      for (j=0; j<ALPHABETSIZE; j++) {

        //fprintf(stdout,"i=%d,j=%d\n",i,j);
        resetScore2d(&mscore->tmpScore[i*ALPHABETSIZE+j], &mscore->meta);
        if (order>0) {
          ji=i*ALPHABETSIZE +j;
        ...
        //mscore->diScore[i*ALPHABETSIZE +j].sumP=

        //fprintf(stdout, "%x\n",&mscore->tmpScore[i*power(ALPHABETSIZE,order) +j]);
        ...
        fprintf(stdout, "%d\n",score2[j]);
        fprintf(stdout, "fmp=%d, fm=%d, m=%d, shift=%d, i=%d, j=%d, ji=%d\n",fmp,fm,m,shift,i,j,ji);
            #endif
        ...
        ShiftMultiplyScoreIndex2d(&mscore->tmpScore[i*ALPHABETSIZE +j], 
             ...
             &score2[j],
             //mscore->diScore[i*ALPHABETSIZE +j].sumP,
          mscore->meta.prob(trans[i*ALPHABETSIZE+j], pf), plb1,pub1,clb1,cub1,
             plb2,pub2,clb2,cub2,
             mscore->meta.length);
  ...
  //      fprintf(stdout,"y1=%f yall=%f\n",mscore->tmpScore[j*power(ALPHABETSIZE, order) +i].y[0],
   //     mscore->ScoreBuffer1[i].y[0]);
      }
      #ifndef IN_R
      #ifdef DEBUG
  ...
  //computeTotalScoreDistribution2d(mscore, order);
  //symmetrizeScoreDistribution2d(mscore);
...
//    if (i==12) fprintf(stdout, "%1.3e x ", p);
    ...
    for (n=corder; n< restlength; n++) {
      ...
      nextprefix-=(nextprefix/power(ALPHABETSIZE, order))*power(ALPHABETSIZE,order);
      ...
      p*=ptmp;
    //  if(i==12) fprintf(stdout, "%1.3e x ", trans[prefix*ALPHABETSIZE+cletter]);
...
//        l=restlength-nsubmotif;
    ...
    if (si2<getScoreLowerBound(escore2,corder-1,prefix)) {
      l=restindex-(restindex/power(ALPHABETSIZE,restlength-n))*power(ALPHABETSIZE, restlength-n);
    ...
    if (nsubmotif< escore1->len) {
      mscore->ScoreBuffer1[suffix].y[
       (si2-getScoreLowerBound(escore2,nsubmotif-shift-1,suffix))*mscore->meta.length
      + si1-getScoreLowerBound(escore1, nsubmotif -1, suffix)]+=p;
    } else {
      mscore->ScoreBuffer1[suffix].y[
       (si2-getScoreLowerBound(escore2,nsubmotif-shift-1,suffix))*mscore->meta.length]+=p;
    }
    p1+=p;
    ...
    printSeq(i,nsubmotif);
    fprintf(stdout, " seq2 ");
    printSeq(i-(i/power(ALPHABETSIZE, restlength))*power(ALPHABETSIZE, restlength),restlength);
...
void computeConditionalOverlappingProbabilities(DMatrix *pwm1, DMatrix *pwm2, double *station,
  ...
  double alpha1, alpha2;
    ...
    intervalsize=(maxScoreIntervalSize(&escore1)>maxScoreIntervalSize(&escore2)) ?
    ...
    #ifndef IN_R
    fprintf(stdout, "forward-forward\n");
    #endif
    for (shift=0; shift<pwm1->nrow; shift++) {
      submotiflen=computeSeqenByBruteForce(pwm1->nrow, pwm1->nrow, shift, &escore1,&escore1);
      //fprintf(stdout, "shift=%d nsub=%d ",shift,submotiflen);
      #ifdef DEBUG
      submotiflen=order;
      #endif
    ...
    #ifndef IN_R
    fprintf(stdout, "forward-reverse\n");
    #endif
    for (shift=0; shift<pwm1->nrow; shift++) {
      submotiflen=computeSeqenByBruteForce(pwm1->nrow, pwm2->nrow, shift, &escore1,&escore2);
      #ifdef DEBUG
      submotiflen=order;
      #endif
      if (submotiflen>shift+order) {
...
//      fprintf(stdout, "shift%d: \n",shift);
    ...
    #endif
    for (shift=0; shift<pwm1->nrow; shift++) {
      submotiflen=computeSeqenByBruteForce(pwm2->nrow, pwm1->nrow, shift, &escore2,&escore1);
      #ifdef DEBUG
      submotiflen=order;
      #endif
      if (submotiflen>shift+order) {
...
void scoredist2d_bfinit(char *bgfile, char *pwmfile, 
  char *output_quantile, char *output_dist, char *pv, char *gr, char *sth) {
  DMatrix pwm1, pwm2;
  ...
  dx=(double)atof(gr);
  if (sth!=NULL) {
    threshold=(int)roundl(atof(sth)/dx);
  }
  if (pv!=NULL) {
    pvalue=atof(pv);
  }
    ...
    if (pv!=NULL) computeConditionalOverlappingProbabilities(&pwm1, &pwm2, station, trans, fout, &pvalue, NULL, &dx, gamma, order);
    else computeConditionalOverlappingProbabilities(&pwm1, &pwm2, station, trans, fout, NULL, &threshold, &dx, gamma, order);

    for (i=1;i<pwm1.nrow*3; i++) {
      gamma[i]/=gamma[0];
    }
    #ifdef IN_R
    Free(gamma);
...
//void scoredist2d_bruteforce(char *bgfile, char *pwmfile, 
//  char *output_quantile, char *output_dist, char *gr, 
//    char *sth, char *sshift, char *slen, char *strand) {
void scoredist2d(char *bgfile, char *pwmfile, 
  ...
  ExtremalScore escore1, escore2;
  ExtremalScore uescore1, uescore2;
  int intervalsize, threshold, shift;
  int order, size1, size2;
  double e=1e-30, dx, pvalue, quantile, alpha1, alpha2;
  FILE *f, *fout;
  ...
  if (sth!=NULL) {
    ...
    if (pv!=NULL) {
      pvalue=atof(pv);
      quantile=getQuantileWithIndex1d(&init1d1,getQuantileIndex1d(&init1d1.totalScore,pvalue));
//      quantile=getQuantileWithIndex1d(&init1d2,getQuantileIndex1d(&init1d2.totalScore,pvalue));
      //threshold=getQuantileIndex1d(&init1d.totalScore,pvalue);
      threshold=(int)(quantile/dx);
      alpha1=getProbWithIndex1d(&init1d1, threshold);
      alpha2=getProbWithIndex1d(&init1d2,threshold);
      fprintf(stdout, "pval=%f, th=%f th=%d pvalf=%f pvalr=%f\n", pvalue, quantile,threshold,
    ...
    resetMotifScore2d(&init2d, order);
    computeScoreDistribution2DBruteForce(
          &pwm1, &pwm1, trans, station, &init2d, 1, 0, 
         &escore1,
         &escore1,
         order);

    fprintf(stdout,"forward-forward overlap\n");
    for (shift=0; shift<pwm1.nrow; shift++) {
      resetMotifScore2d(&null, order);
      fprintf(stdout, "shift=%d ",shift);
      computeScoreDistribution2DDP_init(
       &pwm1, &pwm1, trans, station, &null, 
         &escore1, &escore1, shift, shift+1,
         &init1d1, &init2d, order);
      ...
      fprintf(fout, "%1.10f\n",getMarginalProbability2d(&null,order));
      fprintf(stdout, "%1.10f\n",getMarginalProbability2d(&null,order));
      if(pv!=NULL&&shift==0 && ((getMarginalProbability2d(&null,order)-alpha1) > e)) {
        fprintf(stderr, "ERROR: gamma0=%f!=alpha=%f!\n",getMarginalProbability2d(&null,order), alpha1);
        return;
      }
    }
    ...
    resetMotifScore2d(&init2d, order);
    computeScoreDistribution2DBruteForce(
          &pwm1, &pwm2, trans, station, &init2d, 1, 0, 
         &escore1,
         &escore2,
         order);

    fprintf(stdout,"forward-reverse overlap\n");
    for (shift=0; shift<pwm1.nrow; shift++) {
      fprintf(stdout, "shift=%d ",shift);
      resetMotifScore2d(&null, order);
      computeScoreDistribution2DDP_init(
    ...
    resetMotifScore2d(&init2d, order);
    computeScoreDistribution2DBruteForce(
          &pwm2, &pwm1, trans, station, &init2d, 1, 0, 
         &escore2,
         &escore1,
         order);

    fprintf(stdout,"reverse-forward overlap\n");
    for (shift=0; shift<pwm1.nrow; shift++) {
      fprintf(stdout, "shift=%d ",shift);
      resetMotifScore2d(&null, order);
      computeScoreDistribution2DDP_init(
...
void scoredist2d_bruteforce(char *bgfile, char *pwmfile, 
  ...
  DMatrix pwm1, pwm2;
  double *station, *trans;
  MotifScore1d init1d1;
  MotifScore2d null;
  ExtremalScore escore1, escore2;
  ExtremalScore uescore1;
  int intervalsize, threshold, shift;
  int order, size1;
  double e=1e-30, dx, pvalue, quantile, alpha1;
  FILE *f, *fout;
  ...
  threshold=(int)roundl(atof(sth)/dx);
    ...
    if (pv!=NULL) {
      ...
      getProbability1d(&init1d1.totalScore, &init1d1.meta));
      alpha1=getProbWithIndex1d(&init1d1, threshold);
      fprintf(stdout, "pval=%f, th=%f th=%d, alpha=%f\n", pvalue, quantile,threshold, alpha1);
    }
    ...
    for (shift=0; shift<pwm1.nrow; shift++) {
      ...
      resetMotifScore2d(&null, order);
      computeScoreDistribution2DBruteForce(
       &pwm1, &pwm1, trans, station, &null, shift+pwm1.nrow, shift, 
         &escore1,
         &escore1,
         order);
      fprintf(fout, "%1.10f\n",getMarginalProbability2d(&null,order));
      fprintf(stdout, "%1.10f\n",getMarginalProbability2d(&null,order));
      if(pv!=NULL&&shift==0 && (fabs(getMarginalProbability2d(&null,order)-alpha1) > e)) {
        fprintf(stderr, "NERROR=%f\n",getMarginalProbability2d(&null,order)-alpha1);
        fprintf(stderr, "ERROR: gamma0=%f!=alpha=%f!\n",getMarginalProbability2d(&null,order), alpha1);
        return;
      }
    ...
    for (shift=0; shift<pwm1.nrow; shift++) {
      ...
      resetMotifScore2d(&null, order);
      computeScoreDistribution2DBruteForce(
       &pwm1, &pwm2, trans, station, &null, shift+pwm1.nrow, shift, 
         &escore1,
         &escore2,
         order);
      fprintf(fout, "%1.10f\n",getMarginalProbability2d(&null,order));
    ...
    for (shift=0; shift<pwm1.nrow; shift++) {
      ...
      resetMotifScore2d(&null, order);
      computeScoreDistribution2DBruteForce(
       &pwm2, &pwm1, trans, station, &null, shift+pwm1.nrow, shift, 
         &escore2,
         &escore1,
         order);
      fprintf(fout, "%1.10f\n",getMarginalProbability2d(&null,order));
