...
4. a *false positive probability* $\alpha$ to obtain a motif hit in a random sequence
...
which is defined to be the log-likelihood ratio between the PFM and the background.
...
Consequently, the statistical significance for the notif enrichment is based on the
...
background models and uses a refined way to determine the clump size distribution.
...
a motif hit is set to $\alpha=0.001$. The user might change
the stringency level of $\alpha$  using the `motifcounterOption`.
...
To this end, `scoreSequence` returns the scores at each 
...
using `motifHits` that emerge with a stringency level $\alpha$
...
To this end, `motifcounter` provides the `scoreSequenceProfile` 
...
Alternatively, `motifHitProfile` constructs a similar profile on the basis of
the motif hits 
...
will mostly yield similar results, but they come advantages and disadvantages as well.
...
cpdist=compoundPoissonDist(regions,op)
combdist=combinatorialDist(regions,op)
maxhits=80
