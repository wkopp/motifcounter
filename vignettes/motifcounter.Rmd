...
For simplicity we determine an order-$3$ background model on a subset of chromosome 1:
```{r background}
...
menr = motifEnrichment(regions, motifs, bg, genome=genome)
```
...
Comparison with simulated sequence from the background model.
Lets generate four random sequence sets from the background:
```{r}
rndseqs = lapply(1:4, function(x) motifcounter:::generateDNAStringSet(rep(320, 1000), bg))
names(rndseqs) = paste("simulated", 1:4)
rndseqs = DNAStringSetList(rndseqs)
```

```{r}
smenr = motifEnrichment(rndseqs, motifs,bg)
topMotifs(smenr, doplot=TRUE)
```
