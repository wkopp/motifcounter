...
`motifcounterOption` can also be used to adjust the score granularity, which is used
...
See `?motifcounterOption` for more information.
...
```{r eval=FALSE}
motifcounterOption(0.001)

seqs=generateDNAStringSet(rep(200,100),bg)

motif=oct4

# Enrichment test based on the compound Poisson distribution and
# assuming both DNA strands are scanned for motif hits
motifEnrichmentTest(seqs, motif,bg,singlestranded=FALSE, method="compound")

# Enrichment test based on the compound Poisson distribution and
# assuming a single DNA strands is scanned for motif hits
motifEnrichmentTest(seqs, motif,bg,singlestranded=TRUE, method="compound")

# Enrichment test based on the combinatorial model and
# assuming a single DNA strands is scanned for motif hits
motifEnrichmentTest(seqs, motif,bg, singlestranded=FALSE, method="combinatorial")
```


```{r eval=FALSE}
#take a subset of the oct4 peaks
seqs=oct4peaks[1:100]
...
Inspecting the distribution of the number of motif hits might be of interest in order
to assess the effect size of motif enrichment. For example, what are the quantiles of the distribution
and how does the observed number of motif hits related to that?

In this section, we investigate the compound Poisson approximation more closely,
by looking at the distribution of the number of motif hits for a perfectly palindromic
motif. In case, we scan both DNA strands for hits of this motif (at $\alpha=0.001§),
we observe that the only even numbers of hits are possible. This is the case, because
whenever a hit is observe on one strand, it also has to be present on the other strand.


```{r fig.show=TRUE, fig.cap="Palindromic motif", fig.show='hold'}

regions=rep(100,100)
#regions=100*(100-12+1)
#regions=10000
...
simdist=simulateNumHitsDist(motif,bg,regions,100,singlestranded=FALSE)


op=probOverlapHit(motif,bg,singlestranded=FALSE)
dist=compoundPoissonDist(regions,op)
maxhits=100
seqLogo(motif)
plot(0:(maxhits+1), dist$dist[1:(maxhits++2)],col="blue",ylab="Probability",xlab="Num. of hits")

points(simdist$dist,col="gray")
legend(50,0.08,c("comp. Poiss.", "empirical"), col=c("blue","gray"),lty=c(1,1))

```

The plot also contains the empirical distribution of the number of motif hits
which was generated by sampling 100 sequences of length 10kb, to show the similarity
