...
```{r}
library(MotifDb)
motif=as.list(query(query(query(MotifDb,"hsapiens"),"SP1"),"JASPAR_2014"))[[1]]
motif
```

**NOTE:** In order for `motifcounter` to be a valid PFM, all entries must be strictly positive.
As this is not the case for the original *SP1* motif, the package provides the function `normalizeMotif`, which adds
